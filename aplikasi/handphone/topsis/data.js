...
class TopisisHandphone {
    async data(req, res) {
        ...
        const dataAlternatif = await alternatif.getList(payload)
        if (dataAlternatif instanceof Error) return wrapper.responseError(res, dataAlternatif)

        if (dataAlternatif.data.length === 0) {
    ...
    async harga(req, res) {
        ...
        const dataBobot = await bobot.getList()
        const dataAlternatif = await alternatif.getByHarga(payload)
        if (dataAlternatif instanceof Error) return wrapper.responseError(res, dataAlternatif)
    ...
    async spek(req, res) {
        ...
        const dataBobot = await bobot.getList()
        const dataAlternatif = await alternatif.getBySpek(payload)
        if (dataAlternatif instanceof Error) return wrapper.responseError(res, dataAlternatif)
